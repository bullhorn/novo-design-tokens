...
const makeColorScale = (name, color, light, dark) => {
  return {
    [name]: {
      "@": { value: color },
      shade: { value: chroma(color).darken().hex() },
      tint: { value: chroma(color).brighten().hex() },
      contrast: {
        value: apcaContrastColor(name, color, light, dark),
      },
      overlay: { value: chroma(color).alpha(0.1).hex() },
    },
  };
};
...
const makeScaledColors = (colors, light = white, dark = black) => {
  return Object.keys(colors).reduce((ret, name) => {
    const color = colors[name];
    const scaled = makeColorScale(name, color, light, dark);
    return { ...ret, ...scaled };
  }, {});
};
