 ...
 */
const BH2026_SOURCE = "src/tokens/bh2026/subatomic.figma-export.json";

function buildBh2026() {
  const REM_BASE = 10;
  ...
  const SELECTOR = ':root[data-theme="bh2026"], :root.theme-bh2026';
  const ALIAS = /^\{(.+)\}$/;

  // The export is an array of collections: [{ "<name>": { modes: { "<mode>": {…} } } }].
  // Each collection has a single mode today; we take the first.
  const collections = JSON.parse(readFileSync(BH2026_SOURCE, "utf8"));
  ...
  let out = `/* bh2026 theme — generated by build.mjs from ${BH2026_SOURCE}.\n`;
  ...
  writeFileSync("css/variables-bh2026.css", out);
  const total = Object.values(tierLeaves).reduce((a, l) => a + l.length, 0);
  console.log(
    `css/variables-bh2026.css — ${total} tokens` +
